MODULE BITSSMODULE
  ...
  SUBROUTINE COMPUTE_COEF(COORDS1, COORDS2, KE, KD)
    DOUBLE PRECISION, INTENT(IN) :: COORDS1(NOPT), COORDS2(NOPT)
    DOUBLE PRECISION, INTENT(INOUT) :: KE, KD
  ...
  SUBROUTINE BITSS_EG(COORDS, TARGET_DIST, ENERGY, GRAD)
    DOUBLE PRECISION, INTENT(IN) :: COORDS(2*NOPT), TARGET_DIST
  ...
  SUBROUTINE MINIMISE(COORDS1, COORDS2, TARGET_DIST)
    ...
    MAXITER = 10000
    DO ITER = 1, MAXITER
      CALL BITSS_EG(COORDS, TARGET_DIST, E, G)
      IF (MINIMIZE_CHECK_CONVERGENCE(COORDS)) EXIT
      CALL MINIMIZE_STEP(COORDS, GRAD)
    END DO
  ...
  SUBROUTINE BITSS(COORDS1, COORDS2)
    ...
    DO ITER = 1, BITSSMAXITER
      ...
      DO
        ...
        IF (MINIMISATION_CONVERGED) EXIT
        CALL COMPUTE_COEF(COORDS1, COORDS2, KE, KD)
      END DO
